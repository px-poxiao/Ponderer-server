...
命令分成两类：一类由服务端直接处理，另一类会先经过服务端权限检查，再转发给客户端附加包执行。
...
`hand` 表示使用玩家主手物品推断对应的思索场景 ID。

### 客户端桥接命令

这些命令需要玩家安装 Ponderer-server 客户端附加包。服务端会先检查权限、功能开关和允许列表，然后通过 plugin message 交给客户端执行。

| 命令 | 作用 | 主要权限 |
| --- | --- | --- |
| `/ponderer pull [force\|keep_local]` | 从服务器拉取思索场景 | `ponderer.pull` |
| `/ponderer push [force] [sceneId\|hand]` | 上传或更新思索场景 | `ponderer.upload` |
| `/ponderer reload` | 让客户端重新加载相关数据 | 无额外权限 |
| `/ponderer new [itemId\|hand]` | 为物品创建新的思索场景 | `ponderer.upload` |
| `/ponderer list` | 列出客户端可见的场景信息 | 无额外权限 |
...
| `/ponderer convert <to_ponderjs\|from_ponderjs> <sceneId\|hand\|all>` | 转换场景格式 | 无额外权限 |
| `/ponderer unregister_pack <pack>` | 注销客户端场景包 | 无额外权限 |

## 管理员命令

管理员命令使用 `/ponderer-admin`。执行这些命令首先需要 `ponderer.admin`。
